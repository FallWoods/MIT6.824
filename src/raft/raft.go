...
type ApplyMsg struct {
	CommandValid bool // 标识消息内存储的是指令
	Command      interface{}
	CommandIndex int

	// For 2D:
	SnapshotValid bool // 标识消息内存储的是快照
	Snapshot      []byte
	SnapshotIndex int // 快照中最后一条日志的索引
	SnapshotTerm  int // 两用，快照中最后一条日志的任期或CommandIndex对应的指令的任期
}
...
type Raft struct {
	...
	nextIndex  []int // 复制日志到某个follower时，日志开始的索引
	...
	applyCh     chan ApplyMsg // 用于向应用层传递 应该被应用的指令或快照

	condApply *sync.Cond

	// 2D
	snapShot          []byte // 快照
	lastIncludedIndex int    // 快照中日志的最高索引
	lastIncludedTerm  int    // 快照中日志的最高Term
}
...
func (rf *Raft) ResetHeartTimer(timeDuration int) {
	rf.heartTimer.Reset(time.Duration(timeDuration) * time.Millisecond)
}
...
func (rf *Raft) Snapshot(index int, snapshot []byte) {
	...
	// TODO index 一定大于rf.commitIndex吗？
...
func (rf *Raft) Start(command interface{}) (int, int, bool) {
	...
	defer func() {
		...
		// 为了让应用层传下来的指令被尽快提交，每当有新的日志，立即触发一次心跳
...
func (rf *Raft) CommitChecker() {
	...
	for !rf.killed() {
		...
		for rf.commitIndex > rf.lastApplied {
			...
			DPrintf("server %v 准备commit, log = %v:%v", rf.me, rf.lastApplied, rf.log[rf.RealLogIdx(rf.lastApplied)].Cmd)
			rf.applyCh <- *msg
...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	if args.PrevLogIndex < rf.lastIncludedIndex {
		...
		reply.XTerm = -1
		reply.XLen = rf.VirtualLogIdx(len(rf.log)) // follower最后一条日志下一条日志的索引
		isConflict = true
		...
		reply.XIndex = i + 1
		reply.XLen = rf.VirtualLogIdx(len(rf.log)) // follower最后一条日志的索引
		isConflict = true
		...
		for N > rf.commitIndex {
			...
			for i := 0; i < len(rf.peers); i++ {
				...
				if rf.matchIndex[i] >= N && rf.log[rf.RealLogIdx(N)].Term == rf.currentTerm {
					// N有可能自减到snapShot之前的索引导致log出现负数索引越界
	...
	if reply.Term == rf.currentTerm && rf.role == Leader {
		...
		if i == rf.lastIncludedIndex && rf.log[rf.RealLogIdx(i)].Term > reply.XTerm {
			...
			if reply.XIndex <= rf.lastIncludedIndex {
				// XIndex位置也被截断了，添加InstallSnapshot的处理
				rf.nextIndex[serverTo] = rf.lastIncludedIndex
			} else {
